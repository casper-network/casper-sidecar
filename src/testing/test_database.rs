...
impl DatabaseReader for MockDatabase {
    ...
    async fn get_latest_deploy(&self) -> Result<AggregateDeployInfo, DatabaseRequestError> {
        let mut rng = TEST_RNG.lock().unwrap();
        respond_with_ok_or_not_found(AggregateDeployInfo {
    ...
    async fn get_deploy_processed_by_hash(&self, hash: &str) -> Result<DeployProcessed, DatabaseRequestError> {
        let mut rng = TEST_RNG.lock().unwrap();
        respond_with_ok_or_not_found(DeployProcessed {
    ...
    async fn get_step_by_era(&self, era_id: u64) -> Result<Step, DatabaseRequestError> {
        let mut rng = TEST_RNG.lock().unwrap();
        respond_with_ok_or_not_found(Step {
    ...
    async fn get_fault_by_public_key(&self, public_key: &str) -> Result<Fault, DatabaseRequestError> {
        let mut rng = TEST_RNG.lock().unwrap();
        respond_with_ok_or_not_found(Fault {
            era_id: Default::default(),
...
#[cfg(test)]
fn respond_with_ok_or_not_found<T>(ok_value: T, rng: &mut TestRng) -> Result<T, DatabaseRequestError> {
