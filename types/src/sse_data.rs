...
use std::sync::Arc;
pub fn deserialize(raw_data: &str) -> Result<SseData, Error> {
    serde_json::from_str::<SseData>(raw_data).map_err(|err| {
...
pub mod tests {
    ...
    pub fn example_block_added_1_4_10(block_hash: &str, height: &str) -> String {
        ...
        super::deserialize(&raw_block_added).unwrap(); //reserializing to make sure that the raw json string is in correct form
        raw_block_added
    }
}
