...
impl TryFrom<RpcServerConfigTarget> for RpcServerConfig {
    type Error = RpcConfigParseError;
    fn try_from(value: RpcServerConfigTarget) -> Result<Self, Self::Error> {
        let node_client =
            value
                ...
                .try_into()
                .map_err(
                    |e: ParseNodeClientConfigError| RpcConfigParseError::ParseError {
...
#[derive(Error, Debug)]
pub enum RpcConfigParseError {
    #[error("failed to parse rpc config for {} with error: {}", .field_name, .error)]
    ParseError {
...
impl TryFrom<NodeClientConfigTarget> for NodeClientConfig {
    type Error = ParseNodeClientConfigError;
    fn try_from(value: NodeClientConfigTarget) -> Result<Self, Self::Error> {
        let exponential_backoff =
            value
                .exponential_backoff
                .try_into()
                .map_err(|e: ParseExponentialBackoffError| {
                    ParseNodeClientConfigError::ParseError {
...
#[derive(Error, Debug)]
pub enum ParseNodeClientConfigError {
    #[error("failed to parse node client config for {} with error: {}", .field_name, .error)]
    ParseError {
        field_name: &'static str,
        error: String,
    },
...
impl TryFrom<ExponentialBackoffConfigTarget> for ExponentialBackoffConfig {
    type Error = ParseExponentialBackoffError;
    fn try_from(value: ExponentialBackoffConfigTarget) -> Result<Self, Self::Error> {
        let max_attempts = value
            .max_attempts
            .try_into()
            .map_err(
                |e: MaxAttemptsError| ParseExponentialBackoffError::ParseError {
...
#[derive(Error, Debug)]
pub enum ParseExponentialBackoffError {
    #[error("failed to parse exponential backoff config for {} with error: {}", .field_name, .error)]
    ParseError {
        field_name: &'static str,
        error: String,
    },
...
impl MaxAttemptsError {
    pub fn default() -> Self {
        MaxAttemptsError::ParseError {
            error: "Max attempts must be either 'infinite' or a integer > 0".to_string(),
        }
    }
}
...
mod tests {
    ...
    fn test_should_fail_on_other_inputs() {
        ...
        assert_failing_deserialization(r#"" infinite""#);
        let deserialized =
            serde_json::from_str::<MaxAttemptsTarget>(r#"-1"#);
