...
impl JulietNodeClient {
    pub async fn new(config: NodeClientConfig) -> (Self, impl Future<Output = ()>) {
        ...
        let stream =
            Self::connect_with_retries(config.address, config.exponential_backoff.clone()).await;
        ...
        loop {
            match server.next_request().await {
                Ok(None) | Err(_) => {
                    ...
                    let (reader, writer) = Self::connect_with_retries(addr, config.clone())
                        .await
                        .into_split();
    ...
    async fn connect_with_retries(addr: SocketAddr, config: ExponentialBackoffConfig) -> TcpStream {
