...
The casper node 2.x produces a different set of SSE events than the 1.x ones. Also, 1.x nodes used 3 sse endpoints (`/events/sigs`, `/events/deploys`, `/events/main`), while 2.x node exposed all sse events under one firehose endpoint (`/events`).

Generally the changes in 2.x regarding SSE are backwards incompatible to some extend. To harness all the details and collect all the data clients should adapt the new SSE API. However if some clients are not ready or have no need to adapt to the new SSE API, they can use the legacy SSE emulation.
...
## Translating `BlockAdded` event
    ...
    * When the 2.x event emits a V2 block the following rules apply:
        ...
        * `block.body.propser` will be copied from V2 to V1
    ...
    An example of the above rules are:
    Input V2 BlockAdded:
    ```json
    {
        "BlockAdded": {
...
## Translating `TransactionAccepted` event
    * If the event is a V1 variety - it will be unwrapped and passed, so a 2.x event:
    ```json
    {
        "TransactionAccepted": {
            "Deploy": {
    ...
    ```
    * If the event is a V2 variety - it will be omitted so a 2.x event like:
    ```
    {
        "TransactionAccepted": {
            "Version1": {
...
## Translating `TransactionExpired` event
  ...
  * If it's a Version1 variety it will be omitted from legacy SSE streams. So a 2.x `TransactionExpired` event:
  ```json
    {
        "TransactionExpired": {
            "Version1": {
...
When transalting `ExecutionResultV2` (later in this paragraph called `ex_v2`)to legacy `ExecutionResult` (later in this paragraph called `ex_v1`) the following rules apply:
